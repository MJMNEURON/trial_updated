...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 7);
 	...
 	ivoc_help("help ?1 hh2 /Users/jiemei/Desktop/combined_v2/trial/interneuron/x86_64/HH_traub.mod\n");
...
static int  evaluate_fct ( _threadargsprotocomma_ double _lv ) {
   double _la , _lb , _lvNa , _lvK ;
 _lvNa = _lv - vtraubNa ;
   _lvK = _lv - vtraubK ;
   _la = 0.32 * vtrap ( _threadargscomma_ 13.0 - _lvNa , 4.0 ) ;
   _lb = 0.28 * vtrap ( _threadargscomma_ _lvNa - 40.0 , 5.0 ) ;
   tau_m = 1.0 / ( _la + _lb ) / tadj ;
   m_inf = _la / ( _la + _lb ) ;
   _la = 0.128 * Exp ( _threadargscomma_ ( 17.0 - _lvNa ) / 18.0 ) ;
   _lb = 4.0 / ( 1.0 + Exp ( _threadargscomma_ ( 40.0 - _lvNa ) / 5.0 ) ) ;
   tau_h = 1.0 / ( _la + _lb ) / tadj ;
   h_inf = _la / ( _la + _lb ) ;
   _la = 0.032 * vtrap ( _threadargscomma_ 15.0 - _lvK , 5.0 ) ;
   _lb = 0.5 * Exp ( _threadargscomma_ ( 10.0 - _lvK ) / 40.0 ) ;
